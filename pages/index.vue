    ...
    <transition name="fade" mode="out-in">
      <div v-if="showMainUI" key="main-ui">
        ...
        <header class="fixed top-0 left-0 w-full z-20 flex items-center justify-between px-8 py-4 bg-black/40 backdrop-blur-md border-b border-white/10 shadow-lg animate-fade-in">
          <span class="font-bold text-xl tracking-wide bg-gradient-to-r from-blue-400 via-purple-400 to-pink-400 bg-clip-text text-transparent">AdenMGB</span>
      ...
      <div class="max-w-7xl mx-auto">
        ...
        <section class="py-20 flex flex-col gap-16">
          ...
          <div class="relative max-w-3xl mx-auto bg-white/10 backdrop-blur-2xl rounded-3xl shadow-2xl p-10 flex flex-col md:flex-row items-center gap-6 border border-white/20 hover:scale-[1.02] transition-transform duration-300 group" id="about">
            <div class="absolute inset-0 bg-gradient-to-br from-purple-900/30 via-blue-900/20 to-black/80 blur-2xl rounded-3xl pointer-events-none animate-pulse"></div>
            <div class="flex-shrink-0 flex flex-col items-center justify-center w-28 relative z-10">
          ...
          <div class="relative max-w-5xl mx-auto flex flex-col gap-12 my-16 animate-fade-in" id="projects">
            <div class="flex items-center gap-6 mb-4">
              <div class="relative">
                <div class="absolute inset-0 bg-gradient-to-br from-indigo-900/30 via-purple-900/20 to-black/80 blur-2xl rounded-full pointer-events-none animate-pulse"></div>
                <FolderIcon class="w-16 h-16 text-indigo-300 relative z-10" />
              </div>
              <h2 class="text-3xl font-bold bg-gradient-to-r from-indigo-200 via-purple-200 to-pink-200 bg-clip-text text-transparent">Featured Repos & Contributions</h2>
            </div>
            <div v-if="reposLoading" class="flex justify-center items-center min-h-[30vh]">
              <span class="text-xl text-gray-300 animate-pulse">Loading repos...</span>
            </div>
            <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div v-for="repo in topRepos" :key="repo.id"
                :class="[
                  'rounded-3xl shadow-2xl p-8 border-2 transition-all duration-300 hover:scale-[1.02] group relative overflow-hidden',
                  'flex flex-col gap-6',
                  languageColor(repo.language),
                  'animated-repo-card'
                ]"
              >
                <div :class="['absolute inset-0 opacity-40 blur-2xl pointer-events-none transition-all duration-700', repo.language ? 'repo-bg-' + repo.language.toLowerCase() : 'repo-bg-default']"></div>
                <div class="flex items-start gap-4 relative z-10">
                  <div class="flex-shrink-0 flex flex-col items-center gap-2">
                    <component :is="getLangIcon(repo.language)" class="w-12 h-12" />
                    <span v-if="repo.language" :class="['px-3 py-1 rounded-full font-semibold text-sm', languageBadge(repo.language)]">{{ repo.language }}</span>
                  </div>
                  <div class="flex-1">
                    <div class="flex items-center gap-2 mb-2">
                      <a :href="repo.html_url" target="_blank" class="text-2xl font-bold hover:text-blue-300 transition-colors">{{ repo.full_name || repo.name }}</a>
                      <span v-if="repo.fork" class="text-xs bg-gray-700/60 text-gray-200 px-2 py-0.5 rounded-full">Fork</span>
                    </div>
                    <p class="text-gray-100 text-lg mb-4">{{ repo.description || 'No description.' }}</p>
                    <div class="flex flex-wrap gap-6 items-center text-sm">
                      ...
                      <span class="flex items-center gap-2 text-green-300">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                          <path d="M5 3a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V5a2 2 0 00-2-2H5zm10 2v.217l-5 3.125-5-3.125V5a1 1 0 011-1h8a1 1 0 011 1zm-1.197 2.618L10 10.382l-3.803-2.764L4 7.618V15a1 1 0 001 1h10a1 1 0 001-1V7.618l-1.197.736z"/>
                        </svg>
                        {{ repo.forks_count }}
                      </span>
                      <span class="flex items-center gap-2 text-pink-300">
                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                          <path d="M6 2a1 1 0 00-1 1v2a1 1 0 001 1h8a1 1 0 001-1V3a1 1 0 00-1-1H6zm8 4H6v2a1 1 0 001 1h6a1 1 0 001-1V6z"/>
                        </svg>
            ...
            <div class="flex justify-end mt-4">
              <a href="https://github.com/AdenMGB?tab=repositories" target="_blank" 
                class="text-lg text-blue-300 hover:text-blue-200 transition-colors flex items-center gap-2 group">
                See all on GitHub
          ...
          <div class="relative max-w-3xl mx-auto bg-white/10 backdrop-blur-2xl rounded-3xl shadow-2xl p-10 border border-white/20 flex flex-col md:flex-row items-center gap-6 animate-fade-in" id="stats">
            <div class="absolute inset-0 bg-gradient-to-br from-green-900/30 via-blue-900/20 to-black/80 blur-2xl rounded-3xl pointer-events-none animate-pulse"></div>
            <div class="flex-shrink-0 flex items-center justify-center w-20 h-20 rounded-full bg-gradient-to-br from-green-500/40 to-blue-500/40 relative z-10">
          ...
          <div class="relative max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 animate-fade-in">
            <div class="relative bg-white/10 backdrop-blur-2xl rounded-3xl shadow-2xl p-10 border border-white/20 flex items-center gap-6 hover:scale-[1.02] transition-transform">
              <div class="absolute inset-0 bg-gradient-to-br from-green-900/30 via-blue-900/20 to-black/80 blur-2xl rounded-3xl pointer-events-none animate-pulse"></div>
              <svg class="w-16 h-16 text-green-400 relative z-10" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7-7.5 11-7.5 11s-7.5-4-7.5-11a7.5 7.5 0 1115 0z"/></svg>
              <div>
                <h2 class="text-2xl font-bold text-green-200 mb-2">Location</h2>
                <p class="text-gray-100 text-xl">Adelaide, Australia</p>
              </div>
            </div>
            <div class="relative bg-white/10 backdrop-blur-2xl rounded-3xl shadow-2xl p-10 border border-white/20 flex items-center gap-6 hover:scale-[1.02] transition-transform">
              <div class="absolute inset-0 bg-gradient-to-br from-yellow-900/30 via-orange-900/20 to-black/80 blur-2xl rounded-3xl pointer-events-none animate-pulse"></div>
              <svg class="w-16 h-16 text-yellow-400 relative z-10" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25M6.621 6.621l-1.591 1.591M3 12h2.25m1.371 5.379l-1.591 1.591M12 21v-2.25m5.379-1.371l1.591 1.591M21 12h-2.25m-1.371-5.379l1.591-1.591M16.5 12a4.5 4.5 0 11-9 0 4.5 4.5 0 019 0z"/></svg>
              <div>
                <h2 class="text-2xl font-bold text-yellow-200 mb-2">School</h2>
                <p class="text-gray-100 text-xl">Thanks to my school for 1 year of free Cursor Pro!</p>
              </div>
            </div>
          </div>

          <!-- Philosophy/Quote Section -->
          <div class="relative max-w-3xl mx-auto bg-white/10 backdrop-blur-2xl rounded-3xl shadow-2xl p-12 border border-white/20 flex flex-col items-center animate-fade-in">
            <div class="absolute inset-0 bg-gradient-to-br from-pink-900/30 via-purple-900/20 to-black/80 blur-2xl rounded-3xl pointer-events-none animate-pulse"></div>
            <svg class="w-20 h-20 text-pink-400 mb-4 relative z-10" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
            ...
            <blockquote class="text-2xl text-pink-200 font-semibold text-center italic relative z-10">AI is a tool, not a replacement. Collaboration and curiosity drive progress.</blockquote>
          </div>

          <!-- Contact Section -->
          <div class="relative max-w-3xl mx-auto bg-white/10 backdrop-blur-2xl rounded-3xl shadow-2xl p-12 border border-white/20 flex flex-col items-center animate-fade-in mt-10" id="contact">
            <div class="absolute inset-0 bg-gradient-to-br from-blue-900/30 via-indigo-900/20 to-black/80 blur-2xl rounded-3xl pointer-events-none animate-pulse"></div>
            <svg class="w-20 h-20 text-blue-400 mb-4 relative z-10" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
            </svg>
            <h2 class="text-3xl font-bold text-blue-200 mb-4 relative z-10">Contact</h2>
            <p class="text-gray-100 text-xl mb-6 text-center relative z-10">Want to get in touch? Reach out via <a href="mailto:aden.lindsay@gmail.com" class="text-blue-300 underline hover:text-blue-200 transition-colors">email</a> or <a href="https://github.com/AdenMGB" target="_blank" class="text-blue-300 underline hover:text-blue-200 transition-colors">GitHub</a>.</p>
          </div>
        </section>
      </div>
    </div>
...
const topRepos = computed(() => repos.value.slice(0, 6))
